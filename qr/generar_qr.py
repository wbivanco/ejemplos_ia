...
def buscar_logo(nombre_logo):
    """
    Busca el logo en la carpeta actual (donde está el script)
    ...
    if Path(nombre_logo).suffix:
        logo_path = carpeta_script / nombre_logo
        if logo_path.exists():
        ...
        for ext in extensiones:
            logo_path = carpeta_script / f"{nombre_logo}{ext}"
            if logo_path.exists():
                return str(logo_path)
...
def generar_qr(url, nombre_archivo=None, nombre_logo=None, color_fill=None, color_back=None):
    ...
    if nombre_logo:
        ...
        if not logo_path:
            ...
            raise FileNotFoundError(
                f"El logo '{nombre_logo}' no se encontró en la carpeta '{carpeta_script}'. "
                f"Por favor, coloca el logo en la misma carpeta que el script con ese nombre."
            )
    ...
    # Obtener la carpeta donde está el script (donde se guardarán los QR)
    carpeta_script = Path(__file__).parent
    ...
    # Construir ruta completa en la carpeta del script
    ruta_completa = carpeta_script / nombre_archivo
    
    # Guardar imagen en la carpeta del script
...
Nota: Los logos deben estar en la misma carpeta que este script
        """
    )
    ...
    try:
        ...
        print(f"\n📎 URL: {args.url}")
        if args.nombre_logo:
            print(f"🖼️  Logo: {args.nombre_logo} (buscando en carpeta actual)")
        if args.color_fill:
